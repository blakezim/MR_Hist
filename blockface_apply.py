...
def def_block(rabbit, block):
    ...
    # Load phi_inv
    phi_inv = io.LoadITKFile(f'{data_dir}{vol_ext}{block}_phi_inv.mhd', device=device)
    ...
    io.SaveITKFile(def_bf, f'{data_dir}{def_ext}difference_volume_deformable.mhd')
...
def compose_blocks(rabbit):
    ...
    for block in block_list:
        ...
        if block == block_list[len(block_list) // 2]:
            ...
            bf = io.LoadITKFile(f'{data_dir}{block}{def_ext}difference_volume_deformable.mhd', device=device)
